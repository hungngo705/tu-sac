...
function doEat(game: InternalGame, seat: Seat, cardIds: string[]): ActionResult {
  ...
  const stealingWithPair =
    takingAnotherPlayersDraw &&
    pending.card.rank !== 'TUONG' &&
    handCards.length === 2 &&
  ...
  // Luật bài bụng: cấm dùng đôi trên tay ăn chẵn nếu phá bộ lẻ cùng màu.
  ...
  if (!stealingWithPair && countTrash(remaining) > countTrash(p.hand)) {
